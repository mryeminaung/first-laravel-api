<x-layout>
    @if (session('update'))
        <p class="text-red-500">{{ session('update') }}</p>
    @endif
    <form class="max-w-xs mx-auto" method="POST" action="{{ route('students.update', $student) }}">
        @csrf
        @method('put')
        @method('patch')
        <div class="mb-5">
        ...
        <div class="mb-5">
            ...
            <select id="student_type" name="student_type_id"
                ...
                @foreach ($types as $type)
                    <option {{ $type->id == $student->student_type_id ? 'selected' : '' }} value="{{ $type->id }}">
                        {{ ucwords($type->desc) }}
                    </option>
                @endforeach
            </select>
